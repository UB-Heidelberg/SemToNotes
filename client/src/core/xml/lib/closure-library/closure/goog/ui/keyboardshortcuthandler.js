...
***REMOVED***
***REMOVED*** Due to a bug in the way that Gecko v1.8 on Mac handles
***REMOVED*** cut/copy/paste key events using the meta key, it is necessary to
***REMOVED*** fake the keydown for the action key (C,V,X) by capturing it on keyup.
***REMOVED*** Because users will often release the meta key a slight moment
***REMOVED*** before they release the action key, we need this variable that will
***REMOVED*** store whether the meta key has been released recently.
...
***REMOVED***
***REMOVED*** Handler for when a keyup event is fired in Mac FF2 (Gecko 1.8).
