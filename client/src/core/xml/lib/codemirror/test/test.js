...
testCM("selectionPos", function(cm) {
  ...
  for (var i = 0, e = selElt.length; i < e; ++i) {
    ...
    var atRight = box.right - outer.left > .8***REMOVED*** lineWidth;
    if (atLeft && atRight) {
      sawMiddle = true;
      is(box.bottom - box.top > 90***REMOVED*** lineHeight, "middle high");
      is(width > .9***REMOVED*** lineWidth, "middle wide");
    } else {
      is(width > .4***REMOVED*** lineWidth, "top/bot wide enough");
      is(width < .6***REMOVED*** lineWidth, "top/bot slim enough");
      if (atLeft) {
        sawBottom = true;
        is(box.top - outer.top > 96***REMOVED*** lineHeight, "bot below");
      } else if (atRight) {
        sawTop = true;
        is(box.top - outer.top < 2.1***REMOVED*** lineHeight, "top above");
      }
    }
  }
...
testCM("doubleScrollbar", function(cm) {
  ...
  is(wrap.offsetWidth - byClassName(wrap, "CodeMirror-lines")[0].offsetWidth <= scrollbarWidth***REMOVED*** 1.5);
});
...
testCM("collapsedRangeCoordsChar", function(cm) {
  ...
  var opts = {collapsed: true, inclusiveLeft: true, inclusiveRight: true***REMOVED***
...
testCM("wrappingAndResizing", function(cm) {
  ...
  for (var step = 10, w = cm.charCoords(Pos(0, 18), "div").right;; w += step) {
    cm.setSize(w);
    if (wrap.offsetHeight <= h0***REMOVED*** (opera_lt10 ? 1.2 : 1.5)) {
...
testCM("measureEndOfLine", function(cm) {
  ...
  for (var step = 10, w = cm.charCoords(Pos(0, 7), "div").right;; w += step) {
    cm.setSize(w);
    if (inner.offsetHeight < 2.5***REMOVED*** lh) {
      if (step == 10) { w -= 10; step = 1; }
      else break;
    }
  }
  ...
  is(endPos.top > lh***REMOVED*** .8, "not at top");
...
testCM("moveVstuck", function(cm) {
  ...
  cm.setValue(val);
  for (var w = cm.charCoords(Pos(0, 26), "div").right***REMOVED*** 2.8;; w += 5) {
    cm.setSize(w);
    if (lines.offsetHeight <= 3.5***REMOVED*** h0) break;
  }
...
testCM("extraKeys", function(cm) {
  ...
  function fakeKey(expected, code, props) {
    ...
    var e = {type: "keydown", keyCode: code, preventDefault: function(){}, stopPropagation: function(){}***REMOVED***
    if (props) for (var n in props) e[n] = props[n];
    outcome = null;
    cm.triggerOnKeyDown(e);
    eq(outcome, expected);
  }
  CodeMirror.commands.testCommand = function() {outcome = "tc";***REMOVED***
  CodeMirror.commands.goTestCommand = function() {outcome = "gtc";***REMOVED***
...
testCM("lineChangeEvents", function(cm) {
  ...
  for (var i = 0; i < 5; ++i) {
    CodeMirror.on(cm.getLineHandle(i), "delete", function(i) {
      return function() {log.push("del " + i);***REMOVED***
    }(i));
    CodeMirror.on(cm.getLineHandle(i), "change", function(i) {
      return function() {log.push("ch " + i);***REMOVED***
    }(i));
  }
