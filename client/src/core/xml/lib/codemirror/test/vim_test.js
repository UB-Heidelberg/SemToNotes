...
'  static char***REMOVED***bufp = buf;\n' +
'  if (n == 0) {  /* buffer is empty***REMOVED***\n' +
'    n = read(0, buf, sizeof buf);\n' +
'    bufp = buf;\n' +
'  }\n' +
'\n' +
'  return (--n >= 0) ? (unsigned char)***REMOVED***bufp++ : EOF;\n' +
' \n' +
'}\n';
...
function copyCursor(cur) {
  return { ch: cur.ch, line: cur.line***REMOVED*****REMOVED***
}
...
testJumplist('jumplist_skip_delted_mark<c-i>',
             ...
             [1,0], [0,2]);
***REMOVED***
***REMOVED*** @param name Name of the test
***REMOVED*** @param keys An array of keys or a string with a single key to simulate.
***REMOVED*** @param endPos The expected end position of the cursor.
***REMOVED*** @param startPos The position the cursor should start at, defaults to 0, 0.
***REMOVED***
function testMotion(name, keys, endPos, startPos) {
  testVim(name, function(cm, vim, helpers) {
    if (!startPos) {
      startPos = { line: 0, ch: 0***REMOVED*****REMOVED***
    }
    cm.setCursor(startPos);
    helpers.doKeys(keys);
    helpers.assertCursorAt(endPos);
...
testVim('gj_gk', function(cm, vim, helpers) {
  ...
  var expectedPos2 = { line: 0, ch: (pos1.ch - 4)***REMOVED*** 2 + 4***REMOVED***
  helpers.doKeys('g', 'j');
