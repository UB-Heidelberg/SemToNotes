  ...
  function DiffView(mv, type) {
    ...
    this.classes = type == "left"
      ...
      : {chunk: "CodeMirror-merge-r-chunk",
         ...
         connect: "CodeMirror-merge-r-connect"***REMOVED***
  }
 ...
 ***REMOVED*****REMOVED***
  ...
  function registerUpdate(dv) {
    var edit = {from: 0, to: 0, marked: []***REMOVED***
    var orig = {from: 0, to: 0, marked: []***REMOVED***
    var debounceChange;
  ...
  function syncScroll(dv, type) {
    ...
    var sInfo = editor.getScrollInfo(), halfScreen = .5***REMOVED*** sInfo.clientHeight, midY = sInfo.top + halfScreen;
    var mid = editor.lineAtHeight(midY, "local");
    var around = chunkBoundariesAround(dv.diff, mid, type == DIFF_INSERT);
    var off = getOffsets(editor, type == DIFF_INSERT ? around.edit : around.orig);
    var offOther = getOffsets(other, type == DIFF_INSERT ? around.orig : around.edit);
    var ratio = (midY - off.top) / (off.bot - off.top);
    var targetPos = (offOther.top - halfScreen) + ratio***REMOVED*** (offOther.bot - offOther.top);

    var botDist, mix;
    // Some careful tweaking to make sure no space is left out of view
    // when scrolling to top or bottom.
    if (targetPos > sInfo.top && (mix = sInfo.top / halfScreen) < 1) {
      targetPos = targetPos***REMOVED*** mix + sInfo.top***REMOVED*** (1 - mix);
    } else if ((botDist = sInfo.height - sInfo.clientHeight - sInfo.top) < halfScreen) {
      var otherInfo = other.getScrollInfo();
      var botDistOther = otherInfo.height - otherInfo.clientHeight - targetPos;
      if (botDistOther > botDist && (mix = botDist / halfScreen) < 1)
        targetPos = targetPos***REMOVED*** mix + (otherInfo.height - otherInfo.clientHeight - botDist)***REMOVED*** (1 - mix);
    }
  ...
  function getOffsets(editor, around) {
    ...
    return {top: editor.heightAtLine(around.before || 0, "local"),
            bot: editor.heightAtLine(bot, "local")***REMOVED***
  }
  ...
  function drawConnectors(dv) {
    ...
    iterateChunks(dv.diff, function(topOrig, botOrig, topEdit, botEdit) {
      ...
      copy.chunk = {topEdit: topEdit, botEdit: botEdit, topOrig: topOrig, botOrig: botOrig***REMOVED***
 ...
 ***REMOVED*****REMOVED***
  ...
  function chunkBoundariesAround(diff, n, nInEdit) {
    ...
    return {edit: {before: beforeE, after: afterE}, orig: {before: beforeO, after: afterO}***REMOVED***
  }
