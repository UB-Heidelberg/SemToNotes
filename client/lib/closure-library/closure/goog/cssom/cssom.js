...
***REMOVED***
***REMOVED*** @fileoverview CSS Object Model helper functions.
***REMOVED*** References:
***REMOVED*** - W3C: http://dev.w3.org/csswg/cssom/
***REMOVED*** - MSDN: http://msdn.microsoft.com/en-us/library/ms531209(VS.85).aspx.
***REMOVED*** @supported in FF3, IE6, IE7, Safari 3.1.2, Chrome
***REMOVED*** TODO(user): Fix in Opera.
***REMOVED*** TODO(user): Consider hacking page, media, etc.. to work.
***REMOVED***     This would be pretty challenging. IE returns the text for any rule
***REMOVED***     regardless of whether or not the media is correct or not. Firefox at
***REMOVED***     least supports CSSRule.type to figure out if it's a media type and then
***REMOVED***     we could do something interesting, but IE offers no way for us to tell.
***REMOVED***
...
***REMOVED***
***REMOVED*** Enumeration of {@code CSSRule} types.
***REMOVED*** @enum {number}
***REMOVED***
...
***REMOVED***
***REMOVED*** Recursively gets all CSS as text, optionally starting from a given
***REMOVED*** CSSStyleSheet.
***REMOVED*** @param {(CSSStyleSheet|StyleSheetList)=} opt_styleSheet The CSSStyleSheet.
***REMOVED*** @return {string} css text.
***REMOVED***
goog.cssom.getAllCssText = function(opt_styleSheet) {
  var styleSheet = opt_styleSheet || document.styleSheets;
  return***REMOVED*****REMOVED*** @type {string}***REMOVED*** (goog.cssom.getAllCss_(styleSheet, true));
***REMOVED***


***REMOVED***
***REMOVED*** Recursively gets all CSSStyleRules, optionally starting from a given
***REMOVED*** CSSStyleSheet.
***REMOVED*** Note that this excludes any CSSImportRules, CSSMediaRules, etc..
***REMOVED*** @param {(CSSStyleSheet|StyleSheetList)=} opt_styleSheet The CSSStyleSheet.
***REMOVED*** @return {Array.<CSSStyleRule>} A list of CSSStyleRules.
***REMOVED***
goog.cssom.getAllCssStyleRules = function(opt_styleSheet) {
  var styleSheet = opt_styleSheet || document.styleSheets;
  return***REMOVED*****REMOVED*** @type {Array.<CSSStyleRule>}***REMOVED*** (
      goog.cssom.getAllCss_(styleSheet, false));
***REMOVED***


***REMOVED***
***REMOVED*** Returns the CSSRules from a styleSheet.
***REMOVED*** Worth noting here is that IE and FF differ in terms of what they will return.
***REMOVED*** Firefox will return styleSheet.cssRules, which includes ImportRules and
***REMOVED*** anything which implements the CSSRules interface. IE returns simply a list of
***REMOVED*** CSSRules.
***REMOVED*** @param {CSSStyleSheet} styleSheet The CSSStyleSheet.
***REMOVED*** @throws {Error} If we cannot access the rules on a stylesheet object - this
***REMOVED***     can  happen if a stylesheet object's rules are accessed before the rules
***REMOVED***     have been downloaded and parsed and are "ready".
***REMOVED*** @return {CSSRuleList} An array of CSSRules or null.
***REMOVED***
...
***REMOVED***
***REMOVED*** Gets all CSSStyleSheet objects starting from some CSSStyleSheet. Note that we
***REMOVED*** want to return the sheets in the order of the cascade, therefore if we
***REMOVED*** encounter an import, we will splice that CSSStyleSheet object in front of
***REMOVED*** the CSSStyleSheet that contains it in the returned array of CSSStyleSheets.
***REMOVED*** @param {(CSSStyleSheet|StyleSheetList)=} opt_styleSheet A CSSStyleSheet.
***REMOVED*** @param {boolean=} opt_includeDisabled If true, includes disabled stylesheets,
***REMOVED***    defaults to false.
***REMOVED*** @return {!Array.<CSSStyleSheet>} A list of CSSStyleSheet objects.
***REMOVED***
goog.cssom.getAllCssStyleSheets = function(opt_styleSheet,
  ...
  if (styleSheet.imports && styleSheet.imports.length) {
    ...
    var cssRuleList = goog.cssom.getCssRulesFromStyleSheet(
       ***REMOVED*****REMOVED*** @type {CSSStyleSheet}***REMOVED*** (styleSheet));
...
***REMOVED***
***REMOVED*** Gets the cssText from a CSSRule object cross-browserly.
***REMOVED*** @param {CSSRule} cssRule A CSSRule.
***REMOVED*** @return {string} cssText The text for the rule, including the selector.
***REMOVED***
...
***REMOVED***
***REMOVED*** Get the index of the CSSRule in it's CSSStyleSheet.
***REMOVED*** @param {CSSRule} cssRule A CSSRule.
***REMOVED*** @param {CSSStyleSheet=} opt_parentStyleSheet A reference to the stylesheet
***REMOVED***     object this cssRule belongs to.
***REMOVED*** @throws {Error} When we cannot get the parentStyleSheet.
***REMOVED*** @return {number} The index of the CSSRule, or -1.
***REMOVED***
...
***REMOVED***
***REMOVED*** We do some trickery in getAllCssStyleRules that hacks this in for IE.
***REMOVED*** If the cssRule object isn't coming from a result of that function call, this
***REMOVED*** method will return undefined in IE.
***REMOVED*** @param {CSSRule} cssRule The CSSRule.
***REMOVED*** @return {CSSStyleSheet} A styleSheet object.
***REMOVED***
...
***REMOVED***
***REMOVED*** Replace a cssRule with some cssText for a new rule.
***REMOVED*** If the cssRule object is not one of objects returned by
***REMOVED*** getAllCssStyleRules, then you'll need to provide both the styleSheet and
***REMOVED*** possibly the index, since we can't infer them from the standard cssRule
***REMOVED*** object in IE. We do some trickery in getAllCssStyleRules to hack this in.
***REMOVED*** @param {CSSRule} cssRule A CSSRule.
***REMOVED*** @param {string} cssText The text for the new CSSRule.
***REMOVED*** @param {CSSStyleSheet=} opt_parentStyleSheet A reference to the stylesheet
***REMOVED***     object this cssRule belongs to.
***REMOVED*** @param {number=} opt_index The index of the cssRule in its parentStylesheet.
***REMOVED*** @throws {Error} If we cannot find a parentStyleSheet.
***REMOVED*** @throws {Error} If we cannot find a css rule index.
***REMOVED***
...
***REMOVED***
***REMOVED*** Cross browser function to add a CSSRule into a CSSStyleSheet, optionally
***REMOVED*** at a given index.
***REMOVED*** @param {CSSStyleSheet} cssStyleSheet The CSSRule's parentStyleSheet.
***REMOVED*** @param {string} cssText The text for the new CSSRule.
***REMOVED*** @param {number=} opt_index The index of the cssRule in its parentStylesheet.
***REMOVED*** @throws {Error} If the css rule text appears to be ill-formatted.
***REMOVED*** TODO(bowdidge): Inserting at index 0 fails on Firefox 2 and 3 with an
***REMOVED***     exception warning "Node cannot be inserted at the specified point in
...
***REMOVED***
***REMOVED*** Cross browser function to remove a CSSRule in a CSSStyleSheet at an index.
***REMOVED*** @param {CSSStyleSheet} cssStyleSheet The CSSRule's parentStyleSheet.
***REMOVED*** @param {number} index The CSSRule's index in the parentStyleSheet.
***REMOVED***
...
***REMOVED***
***REMOVED*** Appends a DOM node to HEAD containing the css text that's passed in.
***REMOVED*** @param {string} cssText CSS to add to the end of the document.
***REMOVED*** @param {goog.dom.DomHelper=} opt_domHelper Optional DOM helper user for
***REMOVED***     document interactions.
***REMOVED*** @return {!Element} The newly created STYLE element.
***REMOVED***
...
***REMOVED***
***REMOVED*** Cross browser method to get the filename from the StyleSheet's href.
***REMOVED*** Explorer only returns the filename in the href, while other agents return
***REMOVED*** the full path.
***REMOVED*** @param {!StyleSheet} styleSheet Any valid StyleSheet object with an href.
***REMOVED*** @throws {Error} When there's no href property found.
***REMOVED*** @return {?string} filename The filename, or null if not an external
***REMOVED***    styleSheet.
***REMOVED***
...
***REMOVED***
***REMOVED*** Recursively gets all CSS text or rules.
***REMOVED*** @param {CSSStyleSheet|StyleSheetList} styleSheet The CSSStyleSheet.
***REMOVED*** @param {boolean} isTextOutput If true, output is cssText, otherwise cssRules.
***REMOVED*** @return {string|!Array.<CSSRule>} cssText or cssRules.
***REMOVED*** @private
***REMOVED***
